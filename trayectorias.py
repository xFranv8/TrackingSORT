...
# create video
trck: dict = myTracker.track("video_stm.mp4", False, False)

im = cv.imread('./stmarc_frames/00002000.jpg')
im = cv.cvtColor(im, cv.COLOR_BGR2RGB)
plt.imshow(im)
plt.show()

im1 = cv.imread('./stmarc_frames/00002000.jpg')
...
for k, v in trck.items():
    ...
    for j in range(len(lista_prueba)):
        ...
        plt.plot(x, y, marker='.', color='yellow', alpha=0.2)
